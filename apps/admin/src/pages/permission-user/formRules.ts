...
/**
 * 用户账号 Form.Item 校验规则（新增用户、登录等共用）。
 */
 ...
 * 生成密码 Form.Item 校验规则。
 *
 * @param {boolean} - 是否必填；编辑用户时可传 false。
 * @param {(value: string) => string} - 可选 transform，编辑态用于跳过占位符。
 * @returns {Rule[]} - 密码校验规则列表。
 */
...
/**
 * 密码 Form.Item 校验规则（新增用户、登录必填）。
 */
export const PASSWORD_RULES = getPasswordRules();
